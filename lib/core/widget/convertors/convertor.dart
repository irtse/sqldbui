...
class Convertor {
  ...
  String description, bool require, bool readOnly, dynamic value, String? url, FormWidgetState comp) {
    ...
    || ((type.contains("time") || type.contains("date")) && readOnly)) { 
        return TextWidget(form : form, schemaName: schemaName, name: name,readOnly: readOnly, value: value, label: label, require: require, type: type,);
      } else if ((type.contains("int") && url == null) || type.contains("double") || type.contains("float") || type.contains("money")) { 
        return NumberWidget(form : form, schemaName: schemaName, name: name,readOnly: readOnly, value: value, label: label, require: require, type: type,);
    ...
    if (form[name] != null) { value = form[name]; }
        ...
        return DateWidget(form : form, type: type, schemaName: schemaName, name: name,readOnly: readOnly, value: value, label: label,);
