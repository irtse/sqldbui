  ...
  @override Widget build(BuildContext context) {
    ...
    return FutureBuilder(future: APIService().get<model.Shallowed>(currentView!.filterPath, firstAPI, null), builder: (BuildContext context, AsyncSnapshot<APIResponse<model.Shallowed>> snapshot) {
      ...
      for (var fieldName in widget.schema.keys) {
        ...
        items.add(Padding( padding: const EdgeInsets.all(10), child: AdvancedSwitch(
              ...
              builder: (BuildContext context, StateSetter setState) {
                return Column( children: [ 
                  ...
                  DropdownButtonFormField<String>( items: dpItems, 
                    value: filterView.containsKey(viewID!) && filterView[viewID] != "" ? filterView[viewID] : null,
                    hint: const Text("select an existing view filter...", overflow: TextOverflow.ellipsis,),
                    style: const TextStyle(fontSize: 14, color: Colors.black),
                    onChanged: (value) { setState(() { 
                      widget.currentFilter = value ?? ""; 
                      for (var fieldName in widget.schema.keys) {
                        if (filterConfs.containsKey(widget.currentFilter)) {
                          colsSchemaValid[viewID!]![fieldName]!.value = filterConfs[widget.currentFilter]!.contains(fieldName);
                        }
                      }
                    },); },
