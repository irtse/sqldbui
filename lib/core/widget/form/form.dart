  ...
  Future<Widget> futureBuild(BuildContext context) async {
      ...
      if (widget.view != null && widget.view!.items.isNotEmpty) {
        ...
        if ((refItem.synthesisPath ?? "") != "") {
            synth =  getSynthesis(refItem.synthesisPath!);
        }
              ...
              builder: (BuildContext cont, AsyncSnapshot<APIResponse<model.View>> snap) {
                if (snap.hasData && snap.data!.data != null && snap.data!.data!.isNotEmpty) {
                  for (var data in snap.data!.data!) {
                    ...
                    var newView = model.View(name: TranslateConstants.dataFormulary,
        ...
        if (refItem.isDraft) {
          states.add(Padding(
            ...
            child: Container( padding: const EdgeInsets.symmetric(vertical: 5, horizontal: 15),
              ...
              child: Text(TranslateConstants.draftT.toLowerCase(), style: const TextStyle(color: Colors.white, fontSize: 10)))
          ));
        }
        for (var fieldName in widget.view!.order) {
            ...
            if (widget.view!.actions.contains("put") && !widget.view!.isEmpty && value != "completed" && value != "dismiss" && value != "refused") {
                positionnedBar.add(Padding( padding: const EdgeInsets.only(left: 20, right: 20), child: FloatingActionButton(
                  ...
                  showDialog(context: context, builder: (builder) => ConfirmBoxWidget(purpose: "validate task", validate: () {
                      ...
                      ActionService.pressed(null, false, currentView!.schemaName, currentView!.actionPath, <String>["id"], currentView!.schema, "put", false, context)();
                    }));
          ...
          || (fieldName == "description" && field.readonly && (refItem.values.containsKey("description") && refItem.values["description"] != null)) 
          || (widget.hideField.contains(fieldName))) { continue; }
          String? mainUrl;
          String? url;
          ...
          double max = (counter > 1 ?
              ((currentWidth - menuSize - 100 > 0 ? currentWidth - menuSize - 100 : 1) / (
                (isSplitted && (currentWidth - menuSize < (widget.subForm ? 1000 : 800)) ? 1 : 3)))
              : currentWidth - menuSize - 100  > 0 ? currentWidth - menuSize - 100 : 1);
            var f = await Convertor.formFieldByType(
              newCacheEntry, context, 
        ...
        if (description != "" && !description.contains("no description")) {
            title.add(Padding( padding: const EdgeInsets.only(left: 50), child: Row( 
                children: [ Padding( padding: const EdgeInsets.only(right: 10), child: Icon(Icons.info_outline, size: 20 , color: Theme.of(context).splashColor)), 
        ...
        if (!widget.subForm) {
          if (widget.view != null && !widget.view!.readOnly) {
            if (widget.view!.actions.contains("put") && !widget.view!.isEmpty && (widget.view?.triggers.where( (e) => e.mode == "mail") ?? [] ).isNotEmpty) {
            ...
            if ((widget.view!.actions.contains("post") && widget.view!.isEmpty) || widget.view!.actions.contains("put")) {
              ...
              if (currentView!.items.isNotEmpty && currentView!.items[0].isDraft) {
              ...
              actions.add(ButtonWidget(
                ...
                color: Theme.of(context).primaryColor, isDraft: currentView!.items.isNotEmpty && currentView!.items[0].isDraft));
            }
          ...
          head.add( 
            Container( width: currentWidth - menuSize > 0 ? currentWidth - menuSize : 0, 
      ...
      if ((widget.view?.isEmpty ?? false)) {
          for (var consent in (widget.view?.consents ?? [] as List<model.Consent>)) {
            additionnal.add(ConsentWidget(consent: consent, value: false));
          }
        }
      additionnal.add(synth);
      ...
      if (!widget.scroll) {
        return Container( margin: EdgeInsets.only(// bottom: widget.subForm ? 30 : 0, 
      ...
      Widget formWrap = Container( decoration: BoxDecoration(color: Theme.of(context).highlightColor ),
        width: ((currentWidth - menuSize) * ratioSplit) > 0 ? 
          ((currentWidth - menuSize) * ratioSplit) : 0,
        child: Container(
          margin: EdgeInsets.only(top: wf == null && header.isEmpty ? 112 : ( wf != null && wf.currentHub ? 200 :  152)), 
          height: currentHeigth - (wf == null && header.isEmpty ? 112 : ( wf != null && wf.currentHub ? 200 :  152)) > 0 ? 
            currentHeigth - (wf == null && header.isEmpty ? 112 : ( wf != null && wf.currentHub ? 200 :  152)) : 0,
      ...
      return Stack( children: [ 
        Row(children: [
          ...
          ...(isSplitted ? [
            Container( 
              ...
              height: currentHeigth - (wf == null && header.isEmpty ? 182 : ( wf != null && wf.currentHub ? 260 :  210)) > 0 ? 
                currentHeigth - (wf == null && header.isEmpty ? 182 : ( wf != null && wf.currentHub ? 260 :  210)) : 0,
        ...
        Positioned( bottom: 30, right: 0, 
          child: Container(width: (currentWidth - menuSize) * (1 - ratioSplit), 
