    ...
    if (currentView != null) {
      if (currentView!.isList == true && homeKey.currentState!.widget.subViewID == null) { 
        ...
        return Stack( children: [ 
          ...
          ActionBarWidget(key: globalActionBar, menu: widget.menu, view: currentView, grid: w, gridKey: globalGridKey, viewKey: globalViewKey), 
        ]);
      } else { 
        DataFormWidget w =  DataFormWidget(key: mainForm, view: currentView);
        return Stack( children: [ 
          Container(margin: const EdgeInsets.only(top: 25), child: w),
          ActionBarWidget(key: globalActionBar, menu: widget.menu, view: currentView, viewKey: globalViewKey, form: w ), 
        ] ); }
    }
